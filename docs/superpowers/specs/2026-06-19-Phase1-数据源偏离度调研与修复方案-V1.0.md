...
> **水位线恢复：** `watermarks.json` 文件损坏或不存在时，`WatermarkStore.load()` 返回 `{}`，采集器自动进入全量模式（watermark=0）。首次采集完成后自动重建水位线文件。`devcontext capture` 输出中追加 `[opencode] Watermarks file not found, performing full scan...` 提示。
...
#### 6.1.1.1 Staging 管理命令

| 命令 | 用途 |
|------|------|
| `devContextMemo staging status` | 查看 staging 批次状态（ready/done/failed） |
| `devContextMemo staging retry-failed` | 批量重置 `status=failed` → `ready`（重跑 daemon 即可再处理） |
| `devContextMemo staging retry-failed <session_id>` | 仅重置指定 session 的 batch |

> **设计意图：** 用户看到 `[N/119] failed` 后，修复问题（如 API key），然后 `devcontext staging retry-failed` + 重启 daemon 即可重试，无需手动编辑 `_meta.yaml`。
...
执行流程（同步完成）：
  ...
  Step 9: 如有 --reason，提交到提炼流水线
          - 提炼 prompt 格式: system_prompt = f"资源描述: {reason}\n\n{文档内容}"
          - reason 作为 LLM 上下文的前置描述，帮助 LLM 理解文档的用途和关注点
...
CREATE TABLE knowledge_knowledge_links (
    ...
    occurred_at      INTEGER NOT NULL,          -- LLM 五分类判定时间（毫秒时间戳）
    created_at       INTEGER NOT NULL,
...
> **TODO (Phase 1.x):** `list` 类型分块尚未实现。当前列表内容被合并到 paragraph 块中，
> 需要在 `_chunk_markdown()` 中增加 `bullet_list_open` / `ordered_list_open` token 的
> 分发分支，整体保留列表为一个 block。见 `src/devcontext/services/resource.py:_chunk_markdown`。
